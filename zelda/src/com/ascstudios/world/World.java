...
import com.ascstudios.entities.Bullet;
...
public class World {
			...
			for(int xx = 0; xx < map.getWidth(); xx++) {
				for(int yy = 0; yy < map.getHeight(); yy++) {
					...
					if(pixelAtual == 0xFFFF0000) {
						...
						//	Bullet
						Game.entities.add(new Bullet(xx*16, yy*16, 16, 16, Entity.AMMO_EN));
	...
	public void render(Graphics g) {
		//	Posição que está minha camera em X e Y, 4 ou 16 é o tamanho do meu tile.
		
		int xstart = Camera.x >> 4; // >> 4 é a msm coisa que dividir por 16
		...
		for(int xx = xstart; xx <= xfinal;xx++) {
			for(int yy = ystart; yy <= yfinal; yy++) {
				if(xx < 0 || yy < 0 || xx >= WIDTH || yy >= HEIGHT) { //	Corrigir erro negativo do mapa.
					continue;
				}
